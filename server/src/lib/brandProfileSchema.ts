...
/** Validates Brand.brandProfile JSON on PUT — matches client BrandProfile shape. */
...
export type BrandProfileParsed = z.infer<typeof brandProfileSchema>;

export function parseBrandProfileForSave(raw: unknown): Prisma.InputJsonValue | null {
  const r = brandProfileSchema.safeParse(raw);
  if (!r.success) {
    throw new Error("INVALID_BRAND_PROFILE");
  }
  return r.data as unknown as Prisma.InputJsonValue;
}
